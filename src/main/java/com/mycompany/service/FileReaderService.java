...
public class FileReaderService {
    ...
    public Map<Integer, Set<Integer>> readStationRouteMapFromFile() {
    ...
    private Map<Integer, Set<Integer>> buildStationRouteMap(BufferedReader reader) {
        Map<Integer, Set<Integer>> stationRouteMap = new HashMap<>();
        reader.lines().skip(1).forEach(line -> processLine(stationRouteMap, line));
        return stationRouteMap;
    }

    private void processLine(Map<Integer, Set<Integer>> stationRouteMap, String line) {
        ...
        splittedLine.stream().skip(1).forEach(station -> {
            if (!stationRouteMap.containsKey(station)) {
                stationRouteMap.put(station, new TreeSet<>());
            }
